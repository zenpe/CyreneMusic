...
/// 描述“我的”页面面包屑节点
class MyPageBreadcrumbItem {
  final String label;
  final VoidCallback? onTap;
  final bool isCurrent;
  final bool isEmphasized;

  const MyPageBreadcrumbItem({
    required this.label,
    this.onTap,
...
/// Fluent UI “我的”页面面包屑导航条
class FluentMyPageBreadcrumbs extends StatelessWidget {
  final List<MyPageBreadcrumbItem> items;
  final EdgeInsetsGeometry padding;

  const FluentMyPageBreadcrumbs({
    super.key,
    required this.items,
  ...
  Widget build(BuildContext context) {
    ...
    TextStyle _resolveStyle(MyPageBreadcrumbItem item) {
      if (item.isCurrent) {
